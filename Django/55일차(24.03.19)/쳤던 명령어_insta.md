...
REST_FRAMEWORK = {
    'DEFAULT_PERMISSION_CLASSES': [
        'rest_framework.permissions.IsAuthenticated', # 모든 페이지는 기본적으로 로그인한 사용자만 가능하게 하겠다.
    ],
...
urlpatterns = [
    ...
    path('api/token/', TokenObtainPairView.as_view(), name='token_obtain_pair'), # jwt 토큰 발급 -> 자동생성됨
    path('api/token/refresh/', TokenRefreshView.as_view(), name='token_refresh'), # jwt 토큰 갱신 -> 자동생성됨
    path('api/schema/', SpectacularAPIView.as_view(), name='schema'), # 스키마 -> 자동생성됨
    path('api/schema/swagger-ui/', SpectacularSwaggerView.as_view(url_name='schema'), name='swagger-ui'), # 스웨거 -> 자동생성됨
    path('api/schema/redoc/', SpectacularRedocView.as_view(url_name='schema'), name='redoc'), # 문서화 -> 자동생성됨
]
...
class User(AbstractUser):
    # 추가 필드 예시
    bio = models.TextField(blank=True) # 사용자 기분 정보 # bio: 한줄 설명같은 기능
    profile_picture = models.ImageField(upload_to='profile_pics/', blank=True)
...
class UserCreateView(generics.CreateAPIView):
    ...
    permission_classes = [permissions.AllowAny] # 이걸 안핳면 settings에 설정한 기본값으로 되기 때문에 오버라이딩한 것


class UserDetailView(generics.RetrieveAPIView): # RetrieveAPIView: Detail을 보기 위한 뷰: Read에 많이 사용
    queryset = User.objects.all()
    serializer_class = UserSerializer

###################################
# users > serializers.py (생성 후 저장) -> 평탄화 작업
...
class UserSerializer(serializers.ModelSerializer):
    class Meta:
        model = User
        fields = ('id', 'username', 'email', 'password', 'bio', 'profile_picture') # __all()__ 사용 금지: 숨겨야할 필드까지 다 보여지기 때문에 직접 입력
        extra_kwargs = {'password': {'write_only': True}}

    def create(self, validated_data):
        user = User.objects.create_user(**validated_data) # ** : 언패킹 작업 시 사용
        return user

###################################
# settings.py
# 우리가 만든 User 모델을 기본 모델로 사용하기 위해 설정
AUTH_USER_MODEL = 'users.User' # 해당 커스텀User를 기본 User모델로 사용하기 위해 설정
...
class Post(models.Model):
    author = models.ForeignKey(User, on_delete=models.CASCADE, related_name='posts')
    caption = models.TextField() # contents
    image = models.ImageField(upload_to='post_images/')
    created_at = models.DateTimeField(auto_now_add=True)
    ...
    def __str__(self):
        ...
        return f'{self.author} - {self.caption[:10]}' # author랑 caption을 같이 보여줌(너무 길면 10글자까지만 보여줌)
...
class Comment(models.Model):
    ...
    def __str__(self):
        return f'{self.author.username} - {self.text[:10]}' # author의 유저명과 text를 같이 보여줌
...
class Like(models.Model):
    ...
    class Meta:
        unique_together = ('post', 'user') # 이렇게 값 두개(post, user)를 묶었을때 유니크한 값으로 만드는 것이다. (한번 누르면 좋아요 두번 누르면 취소작업이니 작동을 안하게)
...
class PostListView(generics.ListAPIView):
    ...
    # settings.py에 이게 기본값으로 설정되어 있는데 이것을 오버라이딩하여 재정의한것.
    # REST_FRAMEWORK = {
    #     'DEFAULT_PERMISSION_CLASSES': [
...
class CommentSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Comment # author와 author_username을 구분한 이유: 프론트엔드에서 username을 사용하려고, 필드를 생성후 get_author_username 함수로 호출
        fields = ['id', 'post', 'author', 'author_username', 'text', 'created_at']
        read_only_fields = ['author']

    def get_author_username(self, obj): # 필드값 author_username를 반환하는 함수
        '''
...
class PostSerializer(serializers.ModelSerializer):
    ...
    def get_isLiked(self, obj):
        ...
        if user.is_authenticated:
            ...
            return Like.objects.filter(post=obj, user=user).exists() # 좋아요를 눌렀는지 아닌지를 확인하여 존재하면 True를 반환(이미 좋아요를 누른 상태)
        return False
